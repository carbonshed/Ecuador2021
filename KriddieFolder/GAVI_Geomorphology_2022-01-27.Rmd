...
eos1_synop <- synop%>%filter(EOS_no=="EOS_1") 
synop_gavi <- rbind(synop_mainstem,eos1_synop)

TrackMap <- qmplot(lon, lat, data = synop_mainstem, zoom = 13,maptype = "toner-background", color=as.factor(Date), shape = EOS_no )

TrackMap <- qmplot(lon, lat, data = synop_gavi, zoom = 13, maptype = "toner-background", color=log10(adjusted_ppm)) +
  scale_color_gradient(name="CO2 (ppm)",
  low = "Blue",
  high = "Red")
...
ggplot(data=df_prediction%>%drop_na(Flux_ave), aes(dist, ele_fit, color=adjusted_ppm)) +
  geom_point()
...
##fix downstream
up <- which.min(abs(df_prediction$dist-429))
...
up <- which.min(abs(df_prediction$dist-347))
down <- which.min(abs(df_prediction$dist-355))
...
up <- which.min(abs(df_prediction$dist-375))
down <- which.min(abs(df_prediction$dist-391))
...
up <- which.min(abs(df_prediction$dist-240))
down <- which.min(abs(df_prediction$dist-250.0963))
...
write.csv(df_prediction, here::here("ProcessedData/GAVI_synoptic_2022-02-14.csv"))
```