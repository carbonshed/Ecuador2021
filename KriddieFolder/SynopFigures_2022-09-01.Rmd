...
bball <- ggplot(df_test%>%filter(Wetland_4 != "Gavilan Outlet")%>%filter(Wetland_4 != "Gavilan Inlet")
  ...
  xlab("Distance [m]") +
 scale_color_discrete(name="Stream Reach",labels=c("Antenas", "Colmillo","Gavilan-mainstem","Tributary 1","Tributary 2")) +
...
bball_zoom <- ggplot(df_test%>%filter(Wetland_4 != "Gavi-mainstem")
  ...
  geom_point(aes(shape = shape,fill=Wetland_4), size=2)+
    ylab(expression(paste("Cumulative CO"[2]," Flux [",mu,"mol ", s^-1,"]"))) + 
  xlab("Distance [m]") +
  scale_color_discrete(name="Stream Reach",labels=c("Antenas", "Colmillo","Gavilan Inlet","Gavilan Outlet","Tributary 1","Tributary 2")) +
