...
Geomorph <- subset(Geomorph,
      time > as.POSIXct('2021-07-27 10:16:00', tz="UTC")&
        time < as.POSIXct('2021-07-27 13:36:00', tz="UTC"))


TrackMap_geo <- qmplot(lon, lat, data = Geomorph, zoom = 13, maptype = "toner-background")
TrackMap_geo
...
Synop_3 <- synop %>%
  filter(Date == "2021-06-22" | Date == "2021-06-23")%>%
  filter(EOS_no == "EOS_2")
...
##fix downstream
up <- which.min(abs(predict$dist-12.28239))
down <- which.min(abs(predict$dist-0.04487809))
...
predict_a <- predict %>% filter(dist < 12.28239)
predict_b <- predict %>% filter(dist > 0)
...
###
df_prediction <- df_prediction_2[order(df_prediction_2$dist),]
...
df_prediction_2 <- loop 

```

```{r ante calc slope, echo=FALSE}

df_prediction <- df_prediction_2[order(df_prediction_2$dist),]
df_prediction$ante_slope <- NA
loop <- df_prediction%>%drop_na(ele_fit)
for(i in 1:nrow(loop)) {       # for-loop over rows
  x = loop$dist[i] - 0
  loop$ele_minus[i] <- loop[which.min(abs(loop$dist-x)),]$ele_fit
  ...
  loop$ante_slope <- (loop$ele_minus-loop$ele_plus)/(loop$dist_minus-loop$dist_plus)
...
ggplot(data=df_prediction_2%>%drop_na(Flux_ave), aes(dist, ele_fit, color=Date)) +
  geom_point()

ggplot(data=df_prediction_2, aes(dist, ele_fit, col="CO2_ppm_ave")) +
...
write.csv(df_prediction_2, here::here("ProcessedData/GAVI_Trib2_synoptic_2022-02-18.csv"))
```
