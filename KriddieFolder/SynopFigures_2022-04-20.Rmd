...
B <- ggplot(df_sats%>%filter(Flux_ave>.05)%>%filter(WatershedSize_ha>20),
  ...
  ylab(expression(paste("CO"[2]," Flux [",mu,"mol ", m^-2, s^-1,"]"))) + 
  labs(linetype="LEGEND", color="LEGEND") +
  scale_fill_discrete(name="Stream Reach",labels=c("Gavilan", "Colmillo")) +
...
Profile <- ggplot(data=df ) +
  geom_line(aes(dist_ANTE, ele_fit), size = 3, color=hex[1]) +
  geom_line(aes(dist_GAVI, ele_fit), size = 3, color=hex[3]) +
  geom_line(aes(dist_COLM, ele_fit), size = 3, color=hex[2]) +
  labs(y="Elevation [m]", x = "Distance [m]")  +
  theme_classic() + My_Theme 


Profile
ggsave("streamprofil_1.png", plot = Profile, device = NULL, path = NULL,
