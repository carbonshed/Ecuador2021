...
for(i in 1:nrow(my_sf_proj)) {
 ...
 #add lat lon of original point so that we can calc difference in accumulation for each point
   ...
   my_sf_proj_select_upstream <- my_sf_proj_select_upstream%>%select(pointid,ele,flo_accu,lat_save,lon_save)%>%rename(ele_up10=ele)%>%rename(lat_up10=lat_save)%>%rename(lon_up10=lon_save)%>%rename(floaccu_up10=flo_accu)
   ...
   my_sf_proj_select_downstream <- my_sf_proj_select_downstream%>%select(pointid,ele,lat_save,lon_save,flo_accu)%>%rename(ele_down10=ele)%>%rename(lat_down10=lat_save)%>%rename(lon_down10=lon_save)%>%rename(floaccu_down10=flo_accu)
   
   my_sf_point <- my_sf_proj[i,]%>%select(pointid,ele,flo_accu,lat_save,lon_save)%>%
...
for(i in 1:nrow(my_sf_proj)) {
  ...
  my_sf_proj_select_upstream <- my_sf_proj_select_upstream[which.min(my_sf_proj_select_upstream$floaccu_diff), ]
  my_sf_proj_select_upstream <- my_sf_proj_select_upstream %>% st_drop_geometry()
  my_sf_proj_select_upstream <- my_sf_proj_select_upstream%>%select(pointid,ele,flo_accu,lat_save,lon_save)%>%rename(ele_up20=ele)%>%rename(lat_up20=lat_save)%>%rename(lon_up20=lon_save)%>%rename(floaccu_up20=flo_accu)
