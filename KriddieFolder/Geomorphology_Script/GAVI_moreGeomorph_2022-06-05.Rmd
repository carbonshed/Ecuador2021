...
```{r read in data}
#df_prediction <- read.csv(here::here("ProcessedData/GAVI_synoptic_2022-04-19.csv"))
df_prediction <- read.csv(here::here("ProcessedData/GAVI_synoptic_2022-06-04.csv"))
df_prediction$X <- NULL

ggplot(data=df_prediction%>%drop_na(Date), aes(dist, ele_fit, color=log10(CO2_ppm_ave)),size=5) +
  geom_point(size=5)
...
XWD <- read.csv(here::here("Geomorphology/Gavilan/GAVImainstem_Geomorph_2022-01-28.csv"))
...
##first above GAVI
XWD_up <- XWD%>%filter(x>600)
for(i in 1:nrow(XWD_up)) {       # for-loop over rows
  if (i == 1) {
    XWD_up$x_diff[i] <- 0
  } else {
    XWD_up[i,"x_diff"] <-   XWD_up[i,"x"] - XWD_up[i-1,"x"]
  }
}
XWD_up <- XWD_up[order(-XWD_up$x),]
XWD_up$x_new <- NA
for(i in 1:nrow(XWD_up)) {       # for-loop over rows
  if (i == 1) {
    XWD_up$x_new[i] <- 0
  } else {
    XWD_up[i,"x_new"] <-   XWD_up[i-1,"x_diff"] + XWD_up[i-1,"x_new"]
  }
}
XWD_up$x_new <-XWD_up$x_new - 0.8
XWD_up$x_new[which(XWD_up$x_new==-0.8)] <- 0

##below GAVI
XWD_down <- XWD%>%filter(x<600)
for(i in 1:nrow(XWD_down)) {       # for-loop over rows
  if (i == 1) {
    XWD_down$x_diff[i] <- 0
  } else {
    XWD_down[i,"x_diff"] <-   XWD_down[i,"x"] - XWD_down[i-1,"x"]
  }
}
XWD_down <- XWD_down[order(-XWD_down$x),]
XWD_down$x_new <- NA
for(i in 1:nrow(XWD_down)) {       # for-loop over rows
  if (i == 1) {
    XWD_down$x_new[i] <- 0
  } else {
    XWD_down[i,"x_new"] <-   XWD_down[i-1,"x_diff"] + XWD_down[i-1,"x_new"]
  }
}

#now add on some distance to account for Gavi_up and the wetland in between

#wetland is dist: 313.0514 to 507.6456
#most downstream measurment of gavi_up is 245.0

XWD_down$x_new <- XWD_down$x_new + (507.6456-313.0514+0.4058) + 245.0

XWD <- rbind(XWD_up,XWD_down)
...
##something a little complicated about GAVi is that there isa wetland where we didn't take measurments from  ______,so that should not be included in the area
    # I think I'll just hard program that in if I can

#where does gavi up empty into the wetland? who knows. lets just add 3 meters 343.420370
#df$midpoint[which(df$x_pred==245.0)] <- (507.645587 + 3)
df$x_pred <- as.numeric(df$x_pred)
df$x_pred <- round(df$x_pred, digits = 1)

df$length[which(df$x_pred==245.0)] <- 179.195152 + 3 - df$midpoint[which(df$x_pred==245)]

df$area[which(df$x_pred==245.0)] <- df$w[which(df$x_pred==245.0)]/100 * df$length[which(df$x_pred==245.0)]

#according to collected geomorph data, wetland outlet is 5m upstream of stn1 @ 445, distance =507.645587
df$midpoint[which(df$x_pred==445.0)] <- 507.645587 - 5 
df$length[which(df$x_pred==445.0)] <- 510.025371 - df$midpoint[which(df$x_pred==445.0)]
df$area[which(df$x_pred==445.0)] <- df$w[which(df$x_pred==445.0)]/100 * df$length[which(df$x_pred==445.0)]



#df$length[which(df$x_pred==100)] <- 109.969768 - 	105.009944
#df$area[which(df$x_pred==100)] <- df$w[which(df$x_pred==100)]/100 * df$length[which(df$x_pred==100)]

#hard code in for last point 
df$length[which(df$x_pred==1009.0)]  <- (1053.341609 - 1046.351081)*2
df$area[which(df$x_pred==1009.0)] <- df$length[which(df$x_pred==1009.0)] *df$w[which(df$x_pred==1009.0)] /100

df_prediction_3 <- df

#write.csv(df_prediction_3, here::here("ProcessedData/Gavi_XWD.csv"))
