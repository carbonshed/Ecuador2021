...
add_coordinates <- data.frame(lat=c(-0.3281818,-0.3281891,-0.3282081,-0.3282233),
                              lon=c(-78.2004517,-78.2004572,-78.2004747,-78.2004911),
                              ele=c(NA,NA,NA,NA),time=c(NA,NA,NA,NA))
...
Geomorph <- rbind(add_coordinates,Geomorph)
...
synop <- synop%>%select(Lon,Lat,Date,DateTime,EOS_no,Flux_ave,Tract,Point,VaisalaType,CO2_ppm_ave,GAVI_waterTempAve,BaroPress_kpa,AirTemp_c,Total_hPa,adjusted_ppm,DOC,TDN)%>%rename(lon=Lat)%>%rename(lat=Lon)%>%rename(WaterTemp_c=GAVI_waterTempAve)
...
synop_mainstem <- synop_mainstem%>%select(lon_fit,lat_fit,Date,EOS_no,Flux_ave,Point,VaisalaType,CO2_ppm_ave,WaterTemp_c,BaroPress_kpa,AirTemp_c,Total_hPa,adjusted_ppm,DOC,TDN)
...
I've decided to split up my script so that I treat upstream and downstream separately
...
#write.csv(df_prediction, here::here("ProcessedData/GAVI_synoptic_2024-09-03.csv"))
#write.csv(check, here::here("ProcessedData/GAVI_synopticpoints_arcpro_2024-09-03.csv"))
...
df_prediction <- read.csv(here::here("ProcessedData/GAVI_synoptic_2024-09-03.csv"))
...
# 2 waypoints snapped to corrdinates that are the same distance. 
#i think that we should delete one of these way points I'll choose the one that is under a culvert, becasue it would make sense that the gps is off when under a culvert

df_prediction_1 <- df_prediction_1%>%filter(notes!="culvert under rd")
...
for(i in 1:nrow(df_prediction_1)) {       # for-loop over rows
  ...
  if (is.na(df_prediction_1[i,"dist"]) == FALSE) {
    ...
    if (x1 < df_prediction_1$x[index]) {
    pred_df_closestremoved <- pred_df%>%filter(x > x1)
...
Now we have a data frame with x predicted and associated with distance. Now to match up the distance with nearest lat/lon
...
write.csv(df_prediction_5, here::here("ProcessedData/GaviDOWN_XWD_2.csv"))
