...
Colmillo is not too far off from a function if lat is the "x" and lon is the 'y'
...
I've decided to split up my script so that I treat upstream and downstream seperatly
...
#df_prediction$dist_diff[1] <- max(df_prediction$dist)
...
check <- df_prediction%>%drop_na(Flux_ave)

#write.csv(df_prediction, here::here("ProcessedData/GAVI_synoptic_2024-10-02.csv"))
...
df_prediction <- read.csv(here::here("ProcessedData/GAVI_synoptic_2024-10-02.csv"))
df_prediction$X <- NULL

XWD <- read.csv(here::here("Geomorphology/Gavilan/GAVI-DOWNmainstem_Geomorph_2024-08-30.csv"))
