...
colnames(synop) <- c("lat","lon","Date","EOS_no","Flux_ave", "Tract","Point","VaisalaType","CO2_ppm_ave","WaterTemp_c","BaroPress_kpa","AirTemp_c","Total_hPa","adjusted_ppm","SampleID","DateTime","DOC","TDN","ID","Notes")
...
plot(Geomorph_sub$lat,Geomorph_sub$lon)
...
x <- SpatialPoints(Geomorph_sub[,c(1:2)])
...
Geomorph_sub$lon_fit <- pred$lon
Geomorph_sub$lat_fit <- pred$lat


df_prediction <- full_join(df_prediction,Geomorph_sub,by=c("lon_fit","lat_fit"))
```
...
synop <- read.csv(here::here("Synoptic/ANTE_2022-06-04.csv"))
...
ggplot(data=df_prediction%>%drop_na(lat_wypt), aes(dist, ele, color=log10(CO2_ppm_ave)),size=5) +
  geom_point(size=5)

ggplot(data=df_prediction%>%drop_na(lat_wypt), aes(lon_fit, lat_fit, color=log10(CO2_ppm_ave)),size=5) +
  geom_point(size=5)
...
```{r write, echo=FALSE}
df_prediction <- df_prediction%>%select(lat_fit,lon_fit,ele_fit,dist,Date,Point,Flux_ave,CO2_ppm_ave,adjusted_ppm,DOC,TDN,AirTemp_c,Total_hPa,VaisalaType,EOS_no)#%>%rename(dist=dist_diff)
df_prediction <- unique(df_prediction)

##check should have 23 synop points 
check <- df_prediction%>%drop_na(Point)

#write.csv(df_prediction, here::here("ProcessedData/ANTE_synoptic_2024-09-04.csv"))
#write.csv(check, here::here("ProcessedData/ANTE_synopticpoints_arcpro_2024-09-04.csv"))
...
df_prediction <- read.csv(here::here("ProcessedData/ANTE_synoptic_2024-09-04.csv"))
df_prediction$X <- NULL

XWD <- read.csv(here::here("Geomorphology/Atenas/Antenas_Geomorph_2024-09-04.csv"))
...
#write.csv(df_prediction_5, here::here("ProcessedData/ANTE_XWD.csv"))
