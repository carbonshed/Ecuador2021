...
Geomorph_plot <- Geomorph
colnames(Geomorph_plot) <- c("latitude","longitude","elevation","Datetime")
fig <- plot_ly(Geomorph_plot, x = ~latitude, y = ~longitude, z = ~elevation, size = 1,color=~elevation
)
fig <- fig %>% add_markers()
...
fig <- fig %>% layout(scene = list(xaxis = list(title = 'latitude'),
                                   ...
                                   zaxis = list(title = 'elevation')))%>% layout(legend=list(title=list(text='<b> Elevation </b>')))
...
plot(Geomorph_sub$lat,Geomorph_sub$lon,main="Fitted Polynomiel Spline",
        xlab="latitude",
        ylab="longitude")
...
#synop <- read.csv(here::here("Synoptic/COLMILLO_2022-04-19.csv"))
synop <- read.csv(here::here("Synoptic/COLMILLO_2022-06-05.csv"))
...
##format for a nice plot
x <- SpatialPoints(synop[,c(1:2)])
y <- SpatialPoints(df_prediction[,c(1:2)]) 
plot(y, col = "lightblue",pch = 19)
...
synop <- synop[,c("Date","EOS_no","Flux_ave","Point","CO2_ppm_ave","adjusted_ppm","lon_fit","lat_fit","AirTemp_c","VaisalaType","Total_hPa","COLM_waterTempAve","DOC","TDN")]
...
#pretty it up for supp data
p <- df_prediction %>% 
  drop_na(ele) %>%
  mutate(smooth = fit$fitted) %>%
  ggplot(aes(dist, ele)) +
  geom_point(size = 3, alpha = .5, color = "grey") +
  geom_line(aes(dist, smooth), color="red", size=3) +
  theme_bw()
...
flux <- ggplot(data=df_prediction %>%drop_na(Flux_ave), aes(dist, ele_fit, color=Flux_ave)) +
  geom_point(size=3) +
  xlab("Distance") + ylab("Fitted Elevation") + 
  theme_bw() +
