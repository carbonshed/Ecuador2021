...
#colm
dataFrame_colm <- read.csv(here::here("ProcessedData/upscaling_datasets/point_Facc_Ele_colmWS_TableToExcel.csv"))%>%rename(ele=Ele)%>%rename(flo_accu=FloAccu)
...
dataFrame_colm <- dataFrame_colm%>%filter(OBJECTID!=619)
...
dataFrame_ante <- dataFrame_ante%>%filter(OBJECTID!=619)
...
for(i in 1:nrow(my_sf_proj)) {
  # Step 4: Create a 10-meter buffer around the reference point
  buffer_min20 <- st_buffer(my_sf_proj[i,], dist = 16)
  buffer_max20 <- st_buffer(my_sf_proj[i,], dist = 24)
...
#write.csv(all_data_final,here::here("ProcessedData/slope_ante_oct1.csv"))
#write.csv(all_data_final,here::here("ProcessedData/slope_colm_oct1.csv"))
#write.csv(all_data_final,here::here("ProcessedData/slope_gavi_oct1.csv"))
#all_data_final_test <- read.csv(here::here("ProcessedData/slope_gavi_oct1.csv"))
