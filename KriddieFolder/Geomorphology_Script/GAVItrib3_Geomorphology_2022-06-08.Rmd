...
#reverse order
unfortunatly gavi trib 3 is going from downstream to up, and we want upstream to down. so reverse this order
...
ggplot(data=df_prediction%>%drop_na(Flux_ave)%>%filter(dist<40), aes(dist, ele_fit, color=CO2_ppm_ave)) +
  geom_point()
...
```{r write, echo=FALSE}
check <- df_prediction%>%drop_na(Date)
write.csv(df_prediction, here::here("ProcessedData/GAVI_Trib3_synoptic_2022-06-08.csv"))
```
