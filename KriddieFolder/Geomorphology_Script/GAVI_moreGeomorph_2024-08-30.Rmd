...
df_prediction_save <- read.csv(here::here("ProcessedData/GAVI_synoptic_2022-06-04.csv"))
df_prediction_save$X <- NULL

ggplot(data=df_prediction_save%>%drop_na(Date), aes(dist, ele_fit, color=log10(CO2_ppm_ave)),size=5) +
  geom_point(size=5)

ggplot(data=df_prediction_save%>%drop_na(Date), aes(lon_fit, lat_fit, color=log10(CO2_ppm_ave)),size=5) +
  geom_point(size=5)
```
...
df_prediction <- arrange.vars(df_prediction_save, c("lon_fit"=1))
