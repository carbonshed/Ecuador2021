...
#df_prediction <- read.csv(here::here("ProcessedData/ANTE_synoptic_2022-04-19.csv"))
df_prediction <- read.csv(here::here("ProcessedData/ANTE_synoptic_2022-06-07.csv"))
...
for(i in 1:nrow(df)) {       # for-loop over rows
  if (i==1) {
    ...
    df$length[i] <- df$midpoint[i+1] - df$midpoint[i]
    df$area[i] <- df$length[i] * df$w[i]/100
  }
}
...
#can i bind in width and depth to this from prediction_3 data frame
df_prediction_5 <- full_join(df_prediction_4, df_prediction_3%>%select(lon_fit,lat_fit,w,d), by=(c("lon_fit","lat_fit")))
```
...
write.csv(df_prediction_4, here::here("ProcessedData/ANTE_synopticGeom_2022-06-07.csv"))
