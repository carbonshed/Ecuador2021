...
df_prediction <- read.csv(here::here("ProcessedData/GAVI_Trib1_synoptic_2022-06-08.csv"))
...
XWD <- read.csv(here::here("Geomorphology/Gavilan/GAVItrib1_2022-08-30.csv"))
...
df_prediction_3 <- df
#write.csv(df_prediction_3, here::here("ProcessedData/Gavitrib1_XWD.csv"))
...
#there's an issue with the first two flux measurments not having a na associated area, so I'm going to manully put that in
width_ave <- XWD%>%
  drop_na(w)%>%
  summarise(mean = mean(w))
synop$surface_area[1] <- as.numeric(width_ave/100 * 25.57650/2)
synop$surface_area[2] <- as.numeric(width_ave/100 * (42.33415-25.57650)/2)
...
ggplot(data=df_prediction_4, aes(dist, ele_fit),size=5) +
  geom_point(size=5)

write.csv(df_prediction_4, here::here("ProcessedData/GAVItrib1_synopticGeom_2022-08-30.csv"))
