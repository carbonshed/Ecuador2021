...
#we have an issue with Antenas data where it goes from upstream to downstream instead of the other way around, so I'm going to swap the distance before wrting out'

df_prediction <- df_prediction[order(df_prediction$dist),]
predict_dist<- df_prediction[,c("dist")] 
predict_dist <- sort(predict_dist, decreasing = TRUE)
df_prediction$dist <- NULL
df_prediction$dist <- predict_dist

write.csv(df_prediction, here::here("ProcessedData/ANTE_synoptic_2022-02-16.csv"))
...
write.csv(WaterChem2, here::here("ProcessedData/ANTE_WaterChem_synop_2022-02-13.csv"))
