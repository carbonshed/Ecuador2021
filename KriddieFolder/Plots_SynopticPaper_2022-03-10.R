...
GAVI$wetland <- "GAVI"
GAVItrib1 <- GAVItrib1[,c("lon_fit","lat_fit","ele_fit","dist","Date","EOS_no","Flux_ave","CO2_ppm_ave","adjusted_ppm")]
GAVItrib1$wetland <- "GAVItrib1"
GAVItrib2 <- GAVItrib2[,c("lon_fit","lat_fit","ele_fit","dist","Date","EOS_no","Flux_ave","CO2_ppm_ave","adjusted_ppm")]
GAVItrib2$wetland <- "GAVItrib2"

GAVItrib2 <- GAVItrib2%>% filter(lat_fit > 	-0.3313318)
#look at fitted river network for fun
geomorph <- rbind(ANTE, GAVI, COLM, GAVItrib1, GAVItrib2)
geomorphGAVI <- rbind(GAVI, GAVItrib1, GAVItrib2)

TrackMap_geo <- qmplot(lon_fit, lat_fit, data = geomorph,
                       zoom = 13, maptype = "toner-background")
TrackMap_geo


TrackMap_geo <- qmplot(lon_fit, lat_fit, data = geomorph%>%drop_na(Flux_ave), 
                       zoom = 13, color = Flux_ave,maptype = "toner-background")
TrackMap_geo
...
fig1 <- plot_ly(data = df %>%filter(K600.effective>42)
               , x = ~log10(adjusted_ppm), y = ~Flux_ave, 
                color=~Wetland)

gfig1 <- ggplot(data=df%>%filter(K600.effective<42)
                ) +
  geom_point(aes(log10(adjusted_ppm), Flux_ave, 
                 color=Wetland))
...
fig5 <- plot_ly(data = df%>%filter(Wetland=="GAVI")
                , x = ~dist, y = ~log10(adjusted_ppm), 
                color=~Wetland, size=3)
...
fig3 <- ggplot(data=df,aes(K600.effective,Flux_ave, color=Wetland)) +
  geom_point(size=3) +
  geom_smooth(method=lm, se=FALSE) + 
