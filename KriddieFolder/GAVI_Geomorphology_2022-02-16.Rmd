...
synop_mainstem <- synop_mainstem[,c("lon_fit","lat_fit","Date","EOS_no","Flux_ave","Point","VaisalaType","CO2_ppm_ave","WaterTemp_c","BaroPress_kpa","AirTemp_c","Total_hPa","adjusted_ppm")]
...
fit <- loess(ele ~ dist, degree=2, span = 0.2, data=df_prediction_1)
...
df_prediction_2 <- df_prediction %>% filter(dist < 600)

distlims <- range(df_prediction_2$dist)
fit <- loess(ele ~ dist, degree=2, span = 0.2, data=df_prediction_2)
...
#fit <- loess(ele ~ dist, degree=2, span = 0.2, data=df_prediction)
...
##fix downstream
up <- which.min(abs(df_prediction$dist-33.5))
down <- which.min(abs(df_prediction$dist-31.5))
...
```{r save  later, echo=FALSE}
fig <- plot_ly(df_prediction, x = ~dist, y = ~ele_fit, size = 1)
fig
## there are problems in the middle too. 

up <- which.min(abs(df_prediction$dist-91.6))
down <- which.min(abs(df_prediction$dist-74))
...
up <- which.min(abs(df_prediction$dist-304))
down <- which.min(abs(df_prediction$dist-242))
...
up <- which.min(abs(df_prediction$dist-900))
down <- which.min(abs(df_prediction$dist-863))
...
up <- which.min(abs(df_prediction$dist-842))
down <- which.min(abs(df_prediction$dist-797))

slope <- (df_prediction[down,"ele_fit"] - df_prediction[up,"ele_fit"])/(df_prediction[down,"dist"] - df_prediction[up,"dist"])
b = df_prediction[up,"ele_fit"] - slope*df_prediction[up,"dist"]
df_prediction_a <- df_prediction %>% 
  filter(dist > 772 & dist < df_prediction[up,"dist"])
df_prediction_b <- df_prediction %>% 
  filter(dist > df_prediction[up,"dist"] | dist < 772)
df_prediction_a$ele_fit <- slope*df_prediction_a$dist + b
df_prediction <- rbind(df_prediction_a,df_prediction_b)
rm(df_prediction_a,df_prediction_b)
...
up <- which.min(abs(df_prediction$dist-1038))
down <- which.min(abs(df_prediction$dist-986))
...
up <- which.min(abs(df_prediction$dist-948.7))
down <- which.min(abs(df_prediction$dist-946.3))
...
write.csv(df_prediction, here::here("ProcessedData/GAVI_synoptic_2022-02-16.csv"))
```
