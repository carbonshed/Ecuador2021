...
##fix downstream
up <- which.min(abs(predict$dist-33.5))
down <- which.min(abs(predict$dist-31.5))

slope <- (predict[down,"ele_fit"] - predict[up,"ele_fit"])/(predict[down,"dist"] - predict[up,"dist"])
b = predict[up,"ele_fit"] - slope*predict[up,"dist"]
predict_a <- predict %>% filter(dist < predict[up,"dist"])
predict_b <- predict %>% filter(dist > predict[up,"dist"])
predict_a$ele_fit <- slope*predict_a$dist + b
predict <- rbind(predict_a,predict_b)
rm(predict_a,predict_b)
...
up <- which.min(abs(predict$dist-96.90593))
down <- which.min(abs(predict$dist-81.54697))
...
up <- which.min(abs(predict$dist-327.2532))
down <- which.min(abs(predict$dist-261.5699))
