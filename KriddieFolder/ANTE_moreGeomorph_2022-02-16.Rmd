...
df_prediction <- read.csv(here::here("ProcessedData/ANTE_synoptic_2022-03-23.csv"))
...
df_prediction_3$midpoint <- NA
df_prediction_3$length <- NA
df_prediction_3$area <- NA

for(i in 1:nrow(df_prediction_3)) {       # for-loop over rows
  if (i==1) {
    df_prediction_3$midpoint[i] <- 0
    df_prediction_3$length[i] <- (df_prediction_3$dist[i+1] + (df_prediction_3$dist[i] - df_prediction_3$dist[i+1])/2) - 0
    df_prediction_3$area[i] <- df_prediction_3$length[i] * df_prediction_3$w[i]/100
    
  } else {
    df_prediction_3$midpoint[i] <- df_prediction_3$dist[i-1] + (df_prediction_3$dist[i] - df_prediction_3$dist[i-1])/2
    df_prediction_3$length[i] <- (df_prediction_3$midpoint[i] - df_prediction_3$midpoint[i-1])
    df_prediction_3$area[i] <- df_prediction_3$length[i] * df_prediction_3$w[i]/100
  }
}
...
synop$total_area <- NULL

for(i in 1:nrow(synop)) { 
  if (i==1){
      synop$total_area[i] <- sum(df_prediction_3$area[ df_prediction_3$dist < synop$midpoint[i]], na.rm = TRUE)
  }else if (i==which(synop$dist==max(synop$dist))){
       synop$total_area[i] <- sum(df_prediction_3$area[ df_prediction_3$dist > synop$midpoint[i-1]], na.rm = TRUE)
      }
  else{
  synop$total_area[i] <- sum(df_prediction_3$area[df_prediction_3$dist > synop$midpoint[i-1] & df_prediction_3$dist < synop$midpoint[i]], na.rm = TRUE)
  }
}

df_prediction_4 <- left_join(df_prediction,synop[,c("lon_fit","lat_fit","dist","Flux_ave","total_area")], by=c("lon_fit","lat_fit","dist","Flux_ave"))

df_prediction_4$total_flux <- df_prediction_4$Flux_ave*df_prediction_4$total_area
...
```{r plot width, echo=FALSE}
ggplot(data=df_prediction_3, aes(dist, color=log10(CO2_ppm_ave)),size=5) +
  geom_point(size=5)
```