...
colnames(synop) <- c("lon","lat","Date","EOS_no","Flux_ave", "Tract","Point","CO2_ppm_ave")
synop <- synop[,c("lat","lon","Date","EOS_no","Flux_ave", "Tract","Point","CO2_ppm_ave")]

Synopjune18 <- synop %>% filter(Date == "2021-06-18")
Synopjune22 <- synop %>% filter(Date == "2021-06-22" & EOS_no == "EOS_1" & Tract == 1)
Synopjune29 <- synop %>% filter(Date == "2021-06-29" & EOS_no == "EOS_2" & Tract == 7)
Synopjune30 <- synop %>% filter(Date == "2021-06-30" & EOS_no == "EOS_2")
...
TrackMap <- qmplot(lon, lat, data = synop_mainstem, zoom = 13,
...
```{r snap synop pts}
synop_mainstem <- synop_mainstem #%>% filter(lat <= -.327)
...
df_prediction <- df_prediction#%>% filter(lat <= -.327)
...
```{r interpolate spline, echo=FALSE}
...
GAVI_co2 <- ggplot(data=df_prediction) +
  ...
  geom_point(data=df_prediction%>%drop_na(Flux_ave)
             , aes(dist, ele_fit, color= CO2_ppm_ave),size=5)
...
df_prediction <- df_prediction %>% filter(dist > 13.5)


fit <- loess(ele ~ dist, degree=2, span = 0.15, data=df_prediction)
