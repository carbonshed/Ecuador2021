...
summary(model3)
model3 <- lm(log1p(catchment_ha)~ log1p(slope_mid), data = df)
summary(model3)
...
p2 <- ggplot(df%>%filter(catchment_ha > 1),aes(x=catchment_ha,y=slope_mid)) +
  annotate("text", x=30, y=1.25, label= expression(atop("p-value < 0.0001",paste(r^{2}," = 0.02"))),size=4) +
  #geom_smooth(method='lm',color="black")+
  geom_point(fill="blue",shape=21,size=3)+ scale_x_continuous(transform = "log1p",breaks=c(3,10,30,100,300))+ scale_y_continuous(transform = "log1p")+ xlab("Catchment size [ha]") + ylab("Slope [m/m]")
...
synop_allsites$site_2 <- factor(synop_allsites$site_2,levels = c("GaviTribs","Ante", "GaviUp", "GaviDown", "Colm"))
k600_df$site_2 <- factor(k600_df$site_2,levels = c("GaviTribs","Ante", "GaviUp", "GaviDown", "Colm"))


my_labels <- c('Tribs', 'Antenas', 'Gavil\u00e1n \nInlet', 'Gavil\u00e1n \nOutlet','Colmillo')

tukey.co2<-TukeyHSD(anova.co2)
tukey.co2

air_ppm <- 416.45
CO2 <- ggplot(synop_allsites, aes(x = site_2, y = adjusted_ppm)) +
  ...
  scale_y_continuous(trans = "log1p", breaks=c(300,1000,3000,10000), labels=c("300","1,000","3,000","10,000"), limits = c(300,25000)) +
  theme_bw(base_size = 16)+
  ylab(expression(paste(italic('p'),"CO"[2]," [ppm]")))  +
    annotate("text", x="Ante", y= 18000,label="B", size = 4) +
  annotate("text", x="Colm", y= 18000,label="B", size = 4) +
  annotate("text", x="GaviUp", y= 18000,label="B", size = 4) +
   annotate("text", x="GaviDown", y= 18000,label="B", size = 4) +
    annotate("text", x="GaviTribs", y= 18000,label="A", size = 4) +
    scale_x_discrete(labels=my_labels) + xlab("")

tukey.k600<-TukeyHSD(anova.k600)
tukey.k600

k600 <- ggplot(k600_df%>%drop_na(x), aes(x = site_2, y = k600_eq1_final)) + 
  geom_boxplot(fill="grey") +
  geom_jitter(shape=18,position=position_jitter(0.1)) + scale_y_continuous(transform="log1p", breaks=c(30,100,300,1000), labels=c("30","100","300","1,000"), limits = c(6,1200)) +
   stat_summary(fun = "mean", geom = "point", shape = 23, size = 3,fill="red") +
  theme_bw(base_size = 16)+
    ylab(expression(paste(italic('k')[600]," [m ", d^-1,"]"))) +
  annotate("text", x="Ante", y= 1000,label="A", size = 4) +
  annotate("text", x="Colm", y= 1000,label="A", size = 4) +
  annotate("text", x="GaviUp", y= 1000,label="B", size = 4) +
  annotate("text", x="GaviDown", y= 1000,label="B", size = 4) +
  annotate("text", x="GaviTribs", y= 1000,label="A", size = 4)+
    scale_x_discrete(labels=my_labels)+ xlab("")
...
flux <- ggplot(k600_df, aes(x = site_2, y = F_mol_m2_d_eq1)) + 
  geom_boxplot(fill="grey") +
  geom_jitter(shape=18,position=position_jitter(0.1)) +
   stat_summary(fun = "mean", geom = "point", shape = 23, size = 3, fill="red") +
  scale_y_continuous(transform = "log1p", 
                     ...
                     limits=c(.1,90)) +
  theme_bw(base_size = 16)+
    ylab(expression(paste(" Evasion [ mol ", m^-2, day^-1,"]")))+
  annotate("text", x="Ante", y= 70,label="B", size = 4) +
  annotate("text", x="Colm", y= 70,label="B", size = 4) +
  annotate("text", x="GaviUp", y= 70,label="AB", size = 4) +
  annotate("text", x="GaviDown", y= 70,label="A", size = 4) +
  annotate("text", x="GaviTribs", y= 70,label="A", size = 4)+
    scale_x_discrete(labels=my_labels)+ xlab("") 
  
slope <- ggplot(k600_df%>%drop_na(x), aes(x = site_2, y = slope_mid)) + 
  geom_boxplot(fill="grey") +
  geom_jitter(shape=18,position=position_jitter(0.1)) +
   stat_summary(fun = "mean", geom = "point", shape = 23, size = 3, color = "black",fill="white") +
  scale_y_log10()

Q <- ggplot(k600_df %>%drop_na(x), aes(x = site_2, y = Q_m3s)) + 
  geom_boxplot(fill="grey") +
  geom_jitter(shape=18,position=position_jitter(0.1)) +
   stat_summary(fun = "mean", geom = "point", shape = 23, size = 3, color = "black",fill="white") +
  scale_y_log10()
...
figure <- ggarrange(CO2 , k600, flux,
                  #  labels = c("A","B","C"),
                    ncol = 1, nrow = 3,
...
bball <- ggplot(df, aes(x=x, y=flux_sum, group=name)) +
  ...
  geom_point(aes(fill=name), size=1,shape=1)+
    ylab(expression(paste("Cumulative CO"[2]," Flux [mol ", day^-1,"]"))) +
  xlab("Distance [m]") + 
...
df_bin5 <- df%>%filter(catchment_ha>=1) %>% mutate(new_bin = cut((catchment_ha), breaks=4))
df_bin5$cathment_name <- factor(df_bin5$cathment_name, levels = c("ante", "gavi", "colm"))

# basic example
p1 <- ggplot(df_bin5, aes(x = log(co2), y = new_bin, fill = cathment_name)) +scale_y_discrete(name="Catchment size [ha]", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) + geom_density_ridges(quantile_lines = TRUE,  quantiles = 2, alpha = 0.5) + theme_ridges() #+ theme(legend.position = "none")

p2 <- ggplot(df_bin5%>%filter(k600_eq1_final<100), aes(x = k600_eq1_final, y = new_bin, fill = cathment_name)) +scale_y_discrete(name="Catchment size [ha]", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) + geom_density_ridges(quantile_lines = TRUE,  quantiles = 2, alpha = 0.5) + theme_ridges() #+ theme(legend.position = "none")

p3 <- ggplot(df_bin5%>%filter(F_mol_m2_d_eq1 < 4 & F_mol_m2_d_eq1 > -2 ), aes(x = F_mol_m2_d_eq1, y = new_bin, fill = cathment_name)) +scale_y_discrete(name="Catchment size [ha]", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) + geom_density_ridges(quantile_lines = TRUE,  quantiles = 2, alpha = 0.5) + theme_ridges() #+ theme(legend.position = "none")

p4 <- ggplot(df_bin5, aes(x = log(F_CO2_molperd_eq1), y = new_bin, fill = cathment_name)) +scale_y_discrete(name="Catchment size [ha]", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) + geom_density_ridges(quantile_lines = TRUE,  quantiles = 2, alpha = 0.5) + theme_ridges() #+ theme(legend.position = "none")
...
histogram <- df%>%filter(catchment_ha>=1)%>%
  ...
  scale_fill_discrete(name = "Catchment", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo")) +
...
df_bin5_summary <- df_bin5%>%group_by(new_bin,cathment_name)%>%filter(catchment_ha>=1)%>%
  summarise(
...
geom_bar(stat="identity", color="black", position=position_dodge())+
   ...
   scale_fill_discrete(name = "", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo")) +
    ylab(expression(paste("Total CO"[2]," Flux [mol ", day^-1,"]"))) +
...
map2_ante <- ggplot(my_sf_2%>%filter(catchment_ha>=1)%>%filter(cathment_name=="ante")%>%filter(F_CO2_molperd_eq1>=0 & F_CO2_molperd_eq1<=1.11470)) + 
  geom_sf(aes(color=F_CO2_molperd_eq1),size=.05) +
  
  scale_color_viridis(name=expression(paste("CO"[2]~"Flux [mol ", day^-1,"]")),option="viridis",trans = "log1p")+
  
   geom_point(data = df%>%filter(catchment_ha>=1)%>%filter(cathment_name=="ante")%>%filter(F_CO2_molperd_eq1>1.11470), aes(x = lon, y = lat), size = .05,  color = "red") +
  
   geom_point(data = df%>%filter(catchment_ha>=1)%>%filter(cathment_name=="ante")%>%filter(F_CO2_molperd_eq1<0), aes(x = lon, y = lat), size = 1,  color = "white") + 
  theme_dark(base_size = 16)+ theme(axis.text.x = element_text(angle = 25, vjust = 1, hjust=1))


map2_gavi <- ggplot(my_sf_2%>%filter(catchment_ha>=1)%>%filter(F_CO2_molperd_eq1>=0 & F_CO2_molperd_eq1<=1.11470)) + 
  geom_sf(aes(color=F_CO2_molperd_eq1),size=.05) +
  scale_color_viridis(name=expression(paste("CO"[2]~"Flux [mol ", day^-1,"]")),option="H",trans = "log1p")+
  theme_gray(base_size = 16)+ theme(axis.text.x = element_text(angle = 25, vjust = 1, hjust=1))
...
full1 <- ggarrange(histogram,barplot,ncol = 2,widths=c(1,2),common.legend = TRUE)
full2 <- ggarrange(map1,map2,ncol = 2)
full3 <- ggarrange(full1,full2,nrow = 2)
