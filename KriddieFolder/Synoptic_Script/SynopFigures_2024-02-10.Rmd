...
#annotate text 
dat_text <- data.frame(
  label = c("p-value = 0.0109", "p-value < .0001", 
            "p-value = 0.0273","p-value = 0.111"),
...
p2 <- ggplot(df%>%drop_na(Wetland)
 ...
 theme_bw() +
  scale_color_discrete(name=NULL,labels=c("Antenas", "Gavil\u00e1n Outlet","Gavil\u00e1n Tributary 1","Gavil\u00e1n Tributary 2"))+
  scale_y_continuous(breaks=c(log10(300),log10(1000), log10(3000), log10(10000)), labels=c("300","1,000","3,000","10,000")) +
...
p3 <- p2 + geom_text(
  data    = dat_text,
  mapping = aes(x = 250, y = -Inf, label = label),
  hjust   = -0.1,
  vjust   = -11
)
...
CO2 <- ggplot(df, aes(x = Wetland_5, y = log10(adjusted_ppm), fill = Wetland_5)) +
  ...
  MyTheme +
  scale_fill_discrete(name=NULL,labels=c("Antenas", "Colmillo","Gavil\u00e1n Inlet", "Gavil\u00e1n Outlet","Gavil\u00e1n Tributaries"))+
  ylab(expression(paste(italic('p'),"CO"[2]," [ppm]"))) +
  scale_y_continuous(breaks=c(log10(300),log10(1000), log10(3000), log10(10000)), labels=c("300","1,000","3,000","10,000"))  +
    annotate("text", x="ANTE", y= log10(18000),label="A", size = 5) +
  annotate("text", x="COLM", y= log10(18000),label="A", size = 5) +
  annotate("text", x="Gavilan Inlet", y= log10(18000),label="A", size = 5) +
   annotate("text", x="Gavilan Outlet", y= log10(18000),label="AB", size = 5) +
    annotate("text", x="GavilanTribs", y= log10(18000),label="B", size = 5) 
...
flux <- ggplot(df, aes(x = Wetland_5, y = Flux_ave, fill = Wetland_5)) + 
  ...
  scale_fill_discrete(name=NULL,labels=c("Antenas", "Colmillo","Gavil\u00e1n Inlet", "Gavil\u00e1n Outlet","Gavil\u00e1n Tributaries")) +
    ...
    annotate("text", x="ANTE", y= 5,label="A", size = 5) +
    annotate("text", x="COLM", y= 5,label="B", size = 5) +
  annotate("text", x="Gavilan Inlet", y= 5,label="AB", size = 5) +
  annotate("text", x="Gavilan Outlet", y= 5,label="AC", size = 5) +
    annotate("text", x="GavilanTribs", y= 5,label="BC", size = 5) 
...
k600 <- ggplot(df%>%filter(K600.effective>-30)
  ...
  annotate("text", x="ANTE", y= 155,label="A", size = 5) +
  annotate("text", x="COLM", y= 155,label="B", size = 5) +
  annotate("text", x="Gavilan Inlet", y= 155,label="A", size = 5) +
  annotate("text", x="Gavilan Outlet", y= 155,label="A", size = 5) +
  annotate("text", x="GavilanTribs", y= 155,label="A", size = 5)
...
A <- ggplot(df%>%filter(CatchmentSize_ha<12.5),
  ...
  scale_fill_manual(name="Stream Reach",labels=c("Antenas", "Gavil\u00e1n Tributary 1","Gavil\u00e1n Tributary 3"), values = c(hex(1)[1], "yellow1", hex(7)[6])) +
   scale_x_continuous(breaks=c(log10(300),log10(1000), log10(3000), log10(10000)), labels=c("300","1,000","3,000","10,000")) + 
  annotate("text", x = log10(7000), y = 2.5, label  = expression("p-value < .0001\n", r^2 ~ "=" ~ .52), size = 4) +
   theme_bw()  + MyTheme 
...
B <- ggplot(df_sats #%>%filter(Flux_ave>.05)
  ...
  ylab(expression(paste("CO"[2]," Flux [",mu,"mol ", m^-2, s^-1,"]"))) + 
  labs(linetype="LEGEND", color="LEGEND") +
  scale_fill_manual(name="Stream Reach",labels=c( "Colmillo","Gavil\u00e1n","Gavil\u00e1n Tributary 2"),values = c(hex(3)[2], hex(3)[3],"darkorange")) +
  annotate("text", x = 180, y = 3.5, label  = expression("p-value = .0004\n", r^2 ~ "=" ~ .11), size = 4) +
   theme_bw()  + MyTheme
...
p1<-ggplot(data=df_bins, aes(x=Catchment_bin, y=k600_stdev)) +
  geom_bar(stat="identity", color="black",fill="grey", width=1)+ 
    ylab(expression(paste("stdev"))) +
...
ggarrange(p2, p1, nrow = 2, common.legend = TRUE)
