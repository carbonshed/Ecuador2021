...
#annotate text 
dat_text <- data.frame(
  label = c("p-value = 0.011\nR-sq = 0.28", "p-value < 0.0001\nR-sq = 0.65", 
            "p-value = 0.025\nR-sq = 0.32","p-value = 0.11\nR-sq = 0.68","p-value = 0.05\nR-sq = 0.57"),
...
p3 <- p2 + geom_text(
  data    = dat_text,
  mapping = aes(x = 450, y = 8000, label = label),size = 10/.pt)
...
p5 <- p4 + geom_text(
  data    = dat_text2,
  mapping = aes(x = 900, y = 3000, label = label),size = 10/.pt)

figure1 <- ggarrange(gaviplot_1, gaviplot_2, ncol = 2)
figure2 <- plot_grid(p3 + rremove("xlab"), p5, ncol = 1,nrow=2,rel_heights = c(5,2))
...
p_Q <- ggplot(Q_df,aes(x=catchment_ha,y=Q_m3s*1000,color="black")) + geom_point(size=3) +  scale_y_continuous(trans = "log10", 
                      breaks=c(.01,.1,1,10,100), 
                      labels=c("0.01","0.1","1.0","10","100")) +     scale_x_continuous(trans = "log10", limits = c(.5,200)) +
...
#  annotate("text", x=5, y=50, label= expression(atop("p-value = < 0.0001",paste(R^{2}," = 0.7"))),size=4) +
  annotate("text", x=5, y=50, label= "p-value = < 0.0001\nR-sq = 0.7",size=4) +
...
#  annotate("text", x=1, y=700, label= expression(atop("p-value = < 0.0001",paste(R^{2}," = 0.34"))),size=4) + 
  annotate("text", x=1, y=700, label= "p-value = < 0.0001\n R-sq = 0.34",size=4) +
  theme_bw(base_size = 18)

XWD_allsites$slope_mid_detection <- ifelse(XWD_allsites$slope_mid==0,0.00025,XWD_allsites$slope_mid)

model_d <- lm(log(d) ~ log(catchment_ha) + log(slope_mid_detection), data = XWD_allsites)
summ_model_d <-summary(model_d)
model_d <- lm(log(d) ~ log(catchment_ha) + log1p(slope_mid_detection), data = XWD_allsites)
summ_model_d <-summary(model_d)

p_depth <- ggplot(XWD_allsites%>%drop_na(slope_mid),aes(x=catchment_ha,y=d,fill=log1p(slope_mid))) + 
  geom_point(size=3,shape=21) +
    geom_smooth(method='lm', formula= y~x,color="black")+
  scale_y_log10(name="Depth (cm)") + 
  scale_x_continuous(name= "Catchment area (ha)",trans = "log10", limits = c(.1,200)) +
#  annotate("text", x=1, y=80, label= expression(atop("p-value = < 0.0001",paste(R^{2}," = 0.50"))),size=4) +
  annotate("text", x=1, y=80, label= "p-value = < 0.0001\n R-sq = 0.50",size=4) +
  theme_bw(base_size = 18)+
  scale_fill_continuous(low = "yellow", high = "darkgreen", na.value = NA,name="Slope (m/m)", labels=c(0,.1,.2,.3,.4),  breaks=c(0,log1p(.1),log1p(.2),log1p(.3),log1p(.4))) +
    theme(legend.position="left")


# Extract the legend. Returns a gtable
leg <- get_legend(p_depth)
# Convert to a ggplot and print
leg2 <- as_ggplot(leg)
p_leg_plot <- plot_grid(NULL,leg2,rel_widths = c(1,.5))

p_full_1 <- plot_grid( 
  p_Q + rremove("legend"), p_leg_plot,
                    labels=c("a.",""),ncol = 1, nrow = 2)

p_full_2 <- plot_grid(p_width+ rremove("legend"),
                    p_depth+ rremove("legend"),
                     align="hv",rel_widths = c(1,1),
                    labels=c("b.","c."),ncol = 1, nrow = 2)

p_full_3 <- plot_grid(p_full_1,p_full_2,align="h",ncol = 2)


synop_allsites$slope_mid_detection <- ifelse(synop_allsites$slope_mid==0,0.00025,synop_allsites$slope_mid)

synop_allsites$slope_up_detection <- ifelse(synop_allsites$slope_up==0,0.00025,synop_allsites$slope_up)

model_co2 <- lm(log(adjusted_ppm) ~ log(catchment_ha) + log1p(slope_up_detection), data = synop_allsites%>%drop_na(slope_up))
...
p_co2 <- ggplot() + 
  geom_point(data=synop_allsites%>%drop_na(slope_up),aes(x=catchment_ha,y=adjusted_ppm,fill=slope_up),size=3,shape=21) +
  scale_y_log10() + scale_x_log10()+ 
  geom_smooth(data=synop_allsites%>%drop_na(slope_up),aes(x=catchment_ha,y=adjusted_ppm,fill=slope_up),method='lm', formula= y~x,color="black")+
  

    geom_ellipse(aes(x0 = 80, y0 = 6500, a = 0.15, b = 0.25, angle = 0),color="red") +

  
  scale_fill_continuous(low = "yellow", high = "darkgreen", na.value = NA,name="Slope (m/m)", labels=c(0,.3,.6,1),  breaks=c(0,log1p(.3),log1p(.6),log1p(1))) +  annotate("text", x=8, y=10000, label= expression("p-value = < 0.0001"),size=4) +
...
synop_allsites_nowetland$slope_up_detection <- ifelse(synop_allsites_nowetland$slope_up==0,0.00025,synop_allsites_nowetland$slope_up)
...
model_co2_nowetland <- lm(log(adjusted_ppm) ~ log(catchment_ha) + log1p(slope_up_detection), data = synop_allsites_nowetland%>%drop_na(slope_up_detection))
summ_model_co2_nowetland <-summary(model_co2_nowetland)

p_co2_nowetland_1 <- ggplot(synop_allsites_nowetland%>%drop_na(slope_up),aes(x=catchment_ha,y=adjusted_ppm,fill=slope_up)) + 
...
p_co2_nowetland_2 <- ggplot(synop_allsites_nowetland%>%drop_na(slope_up_detection),aes(x=catchment_ha,y=adjusted_ppm,fill=slope_up_detection)) + 
  geom_point(size=3,shape=21) + scale_y_log10() + scale_x_log10()+ 
  geom_smooth(method='lm', formula= y~x,color="black")+
  scale_fill_gradient(low = "yellow", high = "darkgreen", na.value = NA) +
  annotate("text", x=1, y=10000, label= expression(atop("p-value = < 0.001",paste(r^{2}," = 0.42"))),size=4) +
...
model_emissionco2_nowetland <- lm(log(adjusted_ppm) ~ log(catchment_ha) + log1p(slope_up_detection), data = synop_allsites_nowetland%>%drop_na(slope_up_detection))
summ_model_co2_nowetland <-summary(model_emissionco2_nowetland)
...
df <- read_csv(here::here("ProcessedData/CumulativeFlux_raymondk600_Oct8.csv"))
...
# Extract the legend. Returns a gtable
leg <- get_legend(bball_zoom)

# Convert to a ggplot and print
leg2 <- as_ggplot(leg)


pgroup <- plot_grid(bball+ rremove("ylab")+rremove("legend"),
          leg2 ,ncol=1)

final <- plot_grid(bball_zoom+rremove("legend"),pgroup,nrow=1, rel_widths = c(1,.75))
...
p1 <-ggplot(data=df_bin5#%>%filter(z_score<z_score_break&z_score>-z_score_break)
            , aes(x=new_bin, y=co2,fill=cathment_name)) +
  ...
  scale_fill_manual(name = "Catchment", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo"),values=c("#fee0d2", "#a1d99b", "#756bb1")) +
  ylab(expression(italic(p)~CO[2] ~'(ppm)')) +
  theme_bw(base_size = 14)+ theme(axis.text.x = element_text(angle = 25, vjust = 1, hjust=1))+
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        legend.position = "top")


p2 <-ggplot(data=df_bin5#%>%filter(z_score<z_score_break & z_score>-z_score_break)
            , aes(x=new_bin, y=k600_eq1_final,fill=cathment_name)) + 
  ...
  geom_boxplot(outlier.shape=21, outlier.size = 1.25, outlier.alpha = .5
       ...
       stat_summary(fun = "mean", geom = "point", shape = 23, size = 3, color = "black",fill="red") +
  scale_x_discrete(name="Catchment area (ha)", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) +    scale_fill_manual(name = "Catchment", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo"),values=c("#fee0d2", "#a1d99b", "#756bb1")) +
...
p3 <-ggplot(data=df_bin5#%>%filter(z_score<z_score_break & z_score>-z_score_break)
            , aes(x=new_bin, y=F_mol_m2_d_eq1,fill=cathment_name)) +
   geom_boxplot(outlier.shape=21,outlier.size = 1.25,outlier.alpha = .5
    ) +   
  stat_summary(fun = "mean", geom = "point", shape = 23, size = 3, color = "black",fill="red") +
  scale_x_discrete(name="Catchment area (ha)", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) +
    scale_fill_manual(name = "Catchment", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo"),values=c("#fee0d2", "#a1d99b", "#756bb1")) +
    ylab(expression(paste(CO[2]," emission (mol ",m^-2," ", d^-1,")"))) + scale_y_continuous(transform="log1p",breaks = c(1,3,10),limits =c(0,20)
      ) +
  theme_bw(base_size = 14)+ theme(axis.text.x = element_text(angle = 25, vjust = 1, hjust=1))


p4 <-ggplot(data=df_bin5#%>%filter(z_score<z_score_break & z_score>-z_score_break)
            , aes(x=new_bin, y=F_CO2_molperd_eq1,fill=cathment_name)) +
   geom_boxplot(outlier.shape=21,outlier.size = 1.25,outlier.alpha = .5) +
  stat_summary(fun = "mean", geom = "point", shape = 23, size = 3, color = "black",fill="red") +
  scale_x_discrete(name="Catchment area (ha)", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) +
    scale_fill_manual(name = "Catchment", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo"),values=c("#fee0d2", "#a1d99b", "#756bb1")) + scale_y_continuous(transform="log1p",breaks = c(1,3,10,30,100),limits = c(0,30)
      ) +
    ylab(expression(paste(CO[2]," emission (mol ", d^-1,")"))) +
  theme_bw(base_size = 14)+ theme(axis.text.x = element_text(angle = 25, vjust = 1, hjust=1),legend.position = "top")


p_full_1 <- plot_grid(p1, 
                      p3+rremove("legend"), nrow=2,ncol=1,align = "v")
p_full_2 <- plot_grid(p2, 
                      p4+rremove("legend"), nrow=2,ncol=1,align = "v")

p_full_3 <- plot_grid(p_full_1, 
                      p_full_2, nrow=1,ncol=2,align = "hv")
...
geom_bar(stat="identity", color="black", position=position_dodge())+
  ...
  scale_fill_manual(name = "Catchment", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo"),values=c("#fee0d2", "#a1d99b", "#756bb1")) +
    ylab(expression(paste("Total CO"[2]," emission (mol ", day^-1,")"))) +
...
full <- ggarrange(histogram,NULL,barplot,ncol = 3,widths=c(1,.2,1.75),common.legend = TRUE,legend = "bottom",labels = c("b.","","c."))
