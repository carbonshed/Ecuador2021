...
k600_df$depth <- read.csv(here::here("ProcessedData/AllSynoptic_raymondk600_Oct8.csv"))%>%drop_na(k600_eq1_final)%>%filter(site!="GaviTrib3")

#this is to select just streams that are in range of streams measured in ulseth
test <- k600_df%>%filter(v_ms>=0.00636 & Q_m3s>=0.00080 & slope_mid >= .000002 & depth >= 0.01460*100 & width >= 0.26000*100)
...
#antenas  cos v dist
model1 <- lm(log(adjusted_ppm)~ x_pred, data = synop_allsites %>%filter(site=="Ante"))
summary(model1)
...
p1 <-ggplot(data=df_bin5, aes(x=new_bin, y=co2,fill=cathment_name)) +
  geom_boxplot() + 
...
p2 <-ggplot(data=df_bin5, aes(x=new_bin, y=k600_eq1,fill=cathment_name)) + scale_y_log10() +
  geom_boxplot() +  
    scale_x_discrete(name="Catchment size [ha]", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) +
   scale_fill_discrete(name = "Catchment", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo")) +
  ylab(expression(paste(italic('k')[600]," [m ", d^-1,"]"))) +
...
p3 <-ggplot(data=df_bin5, aes(x=new_bin, y=F_mol_m2_d_eq1,fill=cathment_name)) +
  geom_boxplot() +  
  scale_x_discrete(name="Catchment size [ha]", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) +
   scale_fill_discrete(name = "Catchment", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo")) +
    ylab(expression(paste("CO"[2]," flux [mol ",m^-2," ", d^-1,"]"))) + scale_y_continuous(transform = "log1p", breaks = c(-1,0,3,10,30)) +
  theme_bw(base_size = 16)+ theme(axis.text.x = element_text(angle = 25, vjust = 1, hjust=1))
p3


p4 <-ggplot(data=df_bin5, aes(x=new_bin, y=F_CO2_molperd_eq1,fill=cathment_name)) +
  geom_boxplot() + 
  scale_x_discrete(name="Catchment size [ha]", labels = c('[1.0 - 2.9]','[2.9 - 8.4]','[8.4 - 24.0]','[24.0 - 69.2]','[107 - 202]')) + scale_y_continuous(transform = "log",breaks=c(.1,1,8)) +
   scale_fill_discrete(name = "Catchment", labels = c("Antenas", 'Gavil\u00e1n',"Colmillo")) +
    ylab(expression(paste("CO"[2]," flux [mol ", d^-1,"]"))) +
  theme_bw(base_size = 16)+ theme(axis.text.x = element_text(angle = 25, vjust = 1, hjust=1))
p4
