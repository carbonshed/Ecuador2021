...
##something a little complicated about antenas is that there is no stream between 410 and 365,so that should not be included in the area
...
for(i in 1:nrow(df)) {       # for-loop over rows
  if (i==1) {
    ...
    df$length[i] <- df$midpoint[i] - df$midpoint[i-1]
    df$area[i] <- df$length[i] * df$w[i]/100
  }
}

df$midpoint[which(df$x_pred==40)] <- (53.7102887 - 41.450288)/2 + 41.450288
df$length[which(df$x_pred==40)] <- 53.7102887 - 41.450288
df$area[which(df$x_pred==40)] <- df$w[which(df$x_pred==40)]/100 * df$length[which(df$x_pred==40)]

df$midpoint[which(df$x_pred==95)] <- (108.0099436 - 3)
df$length[which(df$x_pred==95)] <- 3
df$area[which(df$x_pred==95)] <- df$w[which(df$x_pred==95)]/100 * df$length[which(df$x_pred==95)]
...
df_prediction_4 <- df_prediction_4[c(1:2,5,13:31)]
...
write.csv(WaterChem2, here::here("ProcessedData/ANTE_synopticGeom_2022-03-24.csv"))
