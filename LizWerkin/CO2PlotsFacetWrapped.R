...
site_names=site_names[-2] #to get the cambell logger one out 
...
library(purrr)
##Now we can left join all the combined data to make one big data 
...
joined <- left_join(CO2_01_CO2_data, CO2_02_CO2_data, by ="DateTime", suffix = c(".01", ".02"), keep = TRUE) 
joined <- left_join(joined, CO2_03_CO2_data, by ="DateTime",suffix = c(".j", ".03"), keep = TRUE)
joined <- left_join(joined, CO2_04_CO2_data, by ="DateTime", suffix = c(".j", ".04"), keep = TRUE) 
joined <- left_join(joined, CO2_Well1_CO2_data, by ="DateTime", suffix = c(".j", ".W1"), keep = TRUE)
joined <- left_join(joined, CO2_Well2_CO2_data, by = "DateTime", c(".j", ".W2"), keep = TRUE) 


#Now graphhh
ggplot(joined, aes(x=DateTime,y=ppm.01))+ 
  facet_grid(Station ,labeller="label_both", scales = "free_y")+
  labs(x="Date", y="ppm")+ 
  geom_point(aes(y=ppm.01))+
  geom_point(aes(y=ppm.02))+
  geom_point(aes(y=ppm.03))+
  geom_point(aes(y=ppm.04))
