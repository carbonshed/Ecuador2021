...
k600vTime <- ggplot() + 
  geom_ribbon(data=df_corrected, aes(x=DateTime, ymin=0, ymax=adjusted_ppm, linetype = "1"), 
              alpha=.3, 
              fill="steelblue",
              color="steelblue", size = 1 ) +
  geom_point(data=df, aes(x= DateTime, y= Q_m3s, color="darksalmon"), size = 2.5, alpha=.6) +
  scale_y_continuous(
                     name=expression(paste(" ", italic('k')[600] ," [m ", d^-1,"]")),
                     sec.axis = sec_axis(~., name = expression(paste("Discharge [L ", s^-1,"]")))) + 
  xlab("Date")
...
fig <- plot_ly(df_old%>%filter(Station=='stn_01'),
        x = ~DateTime, y= ~adjusted_ppm, type = 'scatter', mode = 'lines+markers')
fig <- fig %>% add_trace(y = ~Q_m3s*100000, name = 'Q',mode = 'lines')
