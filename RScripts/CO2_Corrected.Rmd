...
CO2_02$DateTime <- round_date(CO2_02$DateTime,unit="1 minutes")
CO2_02 <- CO2_02 %>% 
  group_by(DateTime) %>% 
  summarise(ppm = sum(ppm))

CO2_02_time <- as.data.frame(seq(from = as.POSIXct("2021-06-09 12:15:00", tz = "UTC"), to = as.POSIXct("2021-12-30 12:45:00", tz = "UTC"), by='15 min')) # lubridate
colnames(CO2_02_time) <- "DateTime"
CO2_02 <- left_join(CO2_02_time,CO2_02, by="DateTime")
CO2_02 <- CO2_02 %>% drop_na(ppm) %>% filter(ppm > 10)
...
CO2_Well02 <- CO2_Well02 %>%
  filter(DateTime < as.POSIXct("2021-06-14 11:36:00", tz = "UTC") | DateTime > as.POSIXct("2021-06-17 07:21:00", tz = "UTC")) %>%
